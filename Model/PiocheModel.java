		...
		try {	
			...
			try (PreparedStatement stmt = con.prepareStatement("SELECT * FROM Domino;")){
				try (ResultSet rs = stmt.executeQuery()){
					while (rs.next()) { // Rï¿½cupï¿½ration pour tout les dominos dans la BDD
						idPaysage1 = rs.getInt("idPaysage1"); // Rï¿½cupï¿½ration de l'id du paysage1 de chaque domino ï¿½ la liste des ID
						idPaysage2 = rs.getInt("idPaysage2"); // Rï¿½cupï¿½ration de l'id du paysage2 de chaque domino ï¿½ la liste des ID
						idPaysage.add(idPaysage1); // Ajout de l'id du paysage1 de chaque domino ï¿½ la liste des ID
						idPaysage.add(idPaysage2); // Ajout de l'id du paysage2 de chaque domino ï¿½ la liste des ID
					}
				}
			}

			//Requï¿½te pour rï¿½cupï¿½rer tout les paysages ET les couronnes des paysages dans la table Paysage
			try (PreparedStatement stmt = con.prepareStatement("SELECT * FROM TerrainType JOIN Paysage USING(idTerrainType) WHERE idPaysage = ?;")){
				for (int i=0 ; i<96 ; i++) { //Rï¿½cupï¿½ration de tous les paysages
					stmt.setInt(1, idPaysage.get(i));// ? = l'index de la liste idPaysages
					try (ResultSet rs = stmt.executeQuery()){
						while(rs.next()) {
							idTerrainType = rs.getInt("idTerrainType");// Rï¿½cupï¿½ration de l'id du terrain associï¿½ au paysage
							nbCouronne = rs.getInt("nbCouronne");// Rï¿½cupï¿½ration ds couronnes associï¿½ au paysage
							id_CouPaysages.add(idTerrainType);//Ajout ï¿½ la liste concernant les paysages
							id_CouPaysages.add(nbCouronne);//Ajout ï¿½ la liste concernant les paysages
						}
					}
				}
			}
			...
			//Requï¿½te pour rï¿½cupï¿½rer les terrains
			try (PreparedStatement stmt = con.prepareStatement("SELECT * FROM TerrainType WHERE idTerrainType = ? ; ")){
				for (int i=0 ; i<96 ; i++) {//96 pour rï¿½cupï¿½rer tous les terrains des 96 paysages
					int j = i*2;
